...
void pybind_image_definitions(py::module &m) {
    ...
    docstring::ClassMethodDocInject(m, "RGBDImage", "__init__",
                                    map_shared_argument_docstrings);
    auto rgbdm_image = static_cast<py::class_<RGBDMImage, PyGeometry<RGBDMImage>,
                                   std::shared_ptr<RGBDMImage>, Geometry>>(
                    m.attr("RGBDMImage"));
    rgbdm_image
            .def(py::init<>(), "Construct an empty RGBDMImage.")
            .def(py::init<const Image &, const Image &, const Image &, bool>(),
                 "Parameterized constructor", "color"_a, "depth"_a, "mask"_a,
                 "aligned"_a = true)
          
             .def(py::pickle(
                    [](const RGBDMImage &rgbdm) {
                    // __getstate__
                    return py::make_tuple(rgbdm.color_, rgbdm.depth_, rgbdm.mask_, rgbdm.aligned_);
                    },
                    [](py::tuple t) {
                         // __setstate__
                         if (t.size() != 4) {
                         utility::LogError("Cannot unpickle RGBDMImage! Expecting a tuple of size 4.");
                         }
                         return RGBDMImage(t[0].cast<Image>(), t[1].cast<Image>(), t[2].cast<Image>(), t[3].cast<bool>());
                    }
                    ))

             .def_readwrite("color", &RGBDMImage::color_, "The color image.")
             .def_readwrite("depth", &RGBDMImage::depth_, "The depth image.")
             .def_readwrite("mask", &RGBDMImage::mask_, "The mask image.")
             .def_readwrite("aligned_", &RGBDMImage::aligned_,
                            "Are the depth, color, and mask images aligned" 
                            "(same viewpoint and resolution)?")
             .def("get_min_bound", &RGBDMImage::GetMinBound,
                 "Compute min 2D coordinates for the data (always {0, 0}).")
             .def("get_max_bound", &RGBDMImage::GetMaxBound,
                 "Compute max 2D coordinates for the data.")
             .def("to",
                 py::overload_cast<const core::Device &, bool>(&RGBDMImage::To,
                                                               py::const_),
                 "Transfer the RGBDMImage to a specified device.", "device"_a,
                 "copy"_a = false)
            .def("clone", &RGBDMImage::Clone,
                 "Returns a copy of the RGBDMImage on the same device.")
            .def(
                    "cpu",
                    [](const RGBDMImage &rgbdm_image) {
                        return rgbdm_image.To(core::Device("CPU:0"));
                    },
                    "Transfer the RGBDM image to CPU. If the RGBDM image "
                    "is already on CPU, no copy will be performed.")
            .def(
                    "cuda",
                    [](const RGBDImage &rgbdm_image, int device_id) {
                        return rgbdm_image.To(core::Device("CUDA", device_id));
                    },
                    "Transfer the RGBDM image to a CUDA device. If the RGBDM "
                    "image is already "
                    "on the specified CUDA device, no copy will be performed.",
                    "device_id"_a = 0)

            // Conversion.
            .def("to_legacy", &RGBDMImage::ToLegacy,
                 "Convert to legacy RGBDImage type.")
            // Description.
            .def("__repr__", &RGBDMImage::ToString);
}
